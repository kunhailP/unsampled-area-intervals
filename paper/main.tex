...
\item Without pooling, each area's variance estimated from $\nu$ own degrees of freedom, the kernel envelope of Lemma~\ref{lem:dominate} built from area-wise intervals and a binomial bound on the number of misses is $33$--$40\%$ longer than noisy conformal at $\nu = 2$, about equal at $\nu = 50$, and $4.7\%$ and $7.2\%$ shorter at $\nu = 200$ and $1000$ (30 replications per law; grid values). Area-wise variances from a few units therefore destroy the gain, and a variance model with few parameters is needed.
\end{itemize}
