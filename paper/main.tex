...
\begin{itemize}
\item a shape-free rule in the spirit of \citet{zheng2026}: on the order-statistic event $\int \bar g_T\,dG \ge p_k$, so by Markov's inequality $\pr\{\bar g_T(W) < 1 - c\} \le 1 - q$ for $c = (1 - p_k)/(1 - q)$, and the radius is $Tr$ with $\bar g_T(r) = 1 - c$; it needs no shape assumption, and $c = 1/2$ gives the bound of \citet{guille2024}; rank $k = 108$, the best fixed rank among $106$--$110$;
